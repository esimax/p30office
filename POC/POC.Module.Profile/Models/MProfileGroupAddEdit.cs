...
{
    public class MProfileGroupAddEdit : NotifyObjectBase
    {
        ...
        private DBCTProfileGroup DynamicSelectedData { get; set; }
        private DBCTProfileRoot DynamicProfileRoot { get; set; }

        #region CTOR
        public MProfileGroupAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        private bool Validate()
        {
            ...
            var db = DBCTProfileGroup.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicProfileRoot.Oid, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData = new DBCTProfileGroup(uow)
                        {
                            Title = Title.Trim(),
                            Order = DBCTProfileGroup.GetLastOrder(uow, DynamicProfileRoot.Oid) + 1,
                        };

                        var main = DBCTProfileRoot.FindByOid(uow, DynamicProfileRoot.Oid);
                        DynamicSelectedData.ProfileRoot = main;
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBCTProfileGroup.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
        ...
        private void GetDynamicData()
        {
            ...
            DynamicProfileRoot = MainView.DynamicProfileRoot;
        }
