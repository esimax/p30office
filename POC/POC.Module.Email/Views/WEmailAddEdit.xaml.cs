using System;
using System.Threading.Tasks;
using System.Windows;
using System.Windows.Threading;
using POC.Module.Email.Models;
using POL.DB.P30Office;


namespace POC.Module.Email.Views
{
    public partial class WEmailAddEdit
    {
        private MEmailAddEdit Model { get; set; }

        public WEmailAddEdit(DBCTEmail selectedData)
        {
            InitializeComponent();
            DynamicSelectedData = selectedData;

            Loaded += (s, e) =>
            {
                Model = new MEmailAddEdit(this);
                DataContext = Model;
                firstFocused.Focus();
                POL.Lib.Utils.HelperLocalize.SetLanguageToDefault();
                Task.Factory.StartNew(
                    () =>
                    {
                        System.Threading.Thread.Sleep(200);
                        Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal,
                            new Action(() => firstFocused.SelectAll()));
                    });

            };
            Closing += (s, e) =>
            {
                if (ForceUpdate)
                    DialogResult = true;
            };
        }

        protected bool ForceUpdate { get; set; }

        public DBCTEmail DynamicSelectedData { get; set; }
        public Window DynamicOwner { get { return this; } }

        public void DynamicRefocus()
        {
            firstFocused.Focus();
            ForceUpdate = true;
        }
    }
}
