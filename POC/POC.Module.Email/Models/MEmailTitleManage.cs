...
{
    public class MEmailTitleManage : NotifyObjectBase
    {
        ...
        #region CTOR
        public MEmailTitleManage(object mainView)
        {
            MainView = mainView;
        ...
        #region DataList
        private XPCollection<DBBTEmailTitle2> _DataList;
        public XPCollection<DBBTEmailTitle2> DataList
        {
        ...
        #endregion
        #region SelectedData
        private DBBTEmailTitle2 _SelectedData;
        ...
        {
            CommandNew = new RelayCommand(DataNew, () => AMembership.HasPermission(PCOPermissions.BaseTable_EmailTitle_Add));
            CommandEdit = new RelayCommand(DataEdit, () => SelectedData != null && DynamicGridControl.SelectedItems.Count == 1 && AMembership.HasPermission(PCOPermissions.BaseTable_EmailTitle_Edit));
            CommandDelete = new RelayCommand(DataDelete, () => SelectedData != null && AMembership.HasPermission(PCOPermissions.BaseTable_EmailTitle_Delete));
            CommandRefresh = new RelayCommand(DataRefresh, () => true);
            CommandOK = new RelayCommand(OK, () => DynamicIsSelectionMode);

            CommandExport = new RelayCommand(Export, () => AMembership.HasPermission(PCOPermissions.BaseTable_EmailTitle_Export));
            CommandImport = new RelayCommand(Import, () => AMembership.HasPermission(PCOPermissions.BaseTable_EmailTitle_Import));

            CommandHelp = new RelayCommand(Help, () => ConstantPOCHelpURL.POCHelp43 != "");
        }

        private void DataNew()
        {
            var w = new WEmailTitleAddEdit(null) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataEdit()
        {
            var w = new WEmailTitleAddEdit(SelectedData) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataDelete()
        {
            ...
            var list = srh.Select(n => DynamicGridControl.GetRow(n) as DBBTEmailTitle2).ToList();
                ...
                w =>
                {
                    ...
                    foreach (var db in list)
                    {
                        try
                        {
                            ...
                            var db2 = DBBTEmailTitle2.FindByOid(dxs, db.Oid);
        ...
        private void Export()
        {
            var sf = new SaveFileDialog
            {
                ...
                FileName = "EmailTitle.xml"
            };
            if (sf.ShowDialog() != true) return;

            try
            {
                var io = new EmailTitleIO
                             {
                                 Titles = (from n in DBBTEmailTitle2.GetAll(ADatabase.Dxs) select n.Title).ToArray()
                             };
        ...
        private void Import()
        {
            ...
            try
            {
                var serializer = new XmlSerializer(typeof(EmailTitleIO));
                using (var f = new StreamReader(sf.FileName))
                {
                    var io = serializer.Deserialize(f) as EmailTitleIO;
                    if (io == null)
                        throw new Exception("محتوای فایل معتبر نمی باشد.");
                    foreach (var v in io.Titles)
                    {
                        var dbcc = DBBTEmailTitle2.FindDuplicateTitleExcept(ADatabase.Dxs, null, v);
                        if (dbcc != null) continue;
                        dbcc = new DBBTEmailTitle2(ADatabase.Dxs) { Title = v };
                        dbcc.Save();
                    }
                }
                POLMessageBox.ShowInformation("عملیات با موفقیت انجام شد.", DynamicOwner);
        ...
        private void PopulateDataList()
        {
            ...
            var xpc = DBBTEmailTitle2.GetAll(ADatabase.Dxs, ff);
            xpc.LoadAsync();
            DataList = xpc;
        }
        ...
        private void Help()
        {
            System.Diagnostics.Process.Start(ConstantPOCHelpURL.POCHelp43);
        }
