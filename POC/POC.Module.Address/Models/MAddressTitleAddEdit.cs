...
{
    public class MAddressTitleAddEdit : NotifyObjectBase
    {
        ...
        private DBBTAddressTitle2 DynamicSelectedData { get; set; }

        #region CTOR
        public MAddressTitleAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        private bool Validate()
        {
            ...
            var db = DBBTAddressTitle2.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData =new DBBTAddressTitle2(uow)
                        {
                            Title = Title.Trim(),
                        };
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBBTAddressTitle2.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
