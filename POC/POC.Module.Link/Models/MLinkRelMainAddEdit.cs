...
{
    public class MLinkRelMainAddEdit : NotifyObjectBase
    {
        ...
        private DBCTContactRelMain DynamicSelectedData { get; set; }

        #region CTOR
        public MLinkRelMainAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "اطلاعات ارتباط بین پرونده ها"; }
        }
        #endregion
        ...
        private bool Validate()
        {
            ...
            var db = DBCTContactRelMain.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData = new DBCTContactRelMain(uow)
                        {
                            Title = Title.Trim(),
                        };
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBCTContactRelMain.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
