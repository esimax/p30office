...
{
    public class MLinkRelSubAddEdit : NotifyObjectBase
    {
        ...
        private DBCTContactRelSub DynamicSelectedData { get; set; }
        private DBCTContactRelMain DynamicRelMain { get; set; }

        #region CTOR
        public MLinkRelSubAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "اطلاعات ارتباط بین پرونده ها (فرعی)"; }
        }
        #endregion
        ...
        private bool Validate()
        {
            ...
            var db = DBCTContactRelSub.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicRelMain.Oid, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData = new DBCTContactRelSub(uow)
                        {
                            Title = Title.Trim(),
                        };

                        var main = DBCTContactRelMain.FindByOid(uow, DynamicRelMain.Oid);
                        DynamicSelectedData.RelMain = main;
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBCTContactRelSub.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
