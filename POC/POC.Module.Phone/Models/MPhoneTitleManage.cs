...
{
    public class MPhoneTitleManage : NotifyObjectBase
    {
        ...
        #region CTOR
        public MPhoneTitleManage(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "فهرست عناوین شماره تماس"; }
        }
        #endregion
        ...
        #region DataList
        private XPCollection<DBBTPhoneTitle2> _DataList;
        public XPCollection<DBBTPhoneTitle2> DataList
        {
        ...
        #endregion
        #region SelectedData
        private DBBTPhoneTitle2 _SelectedData;
        ...
        {
            var w = new WPhoneTitleAddEdit(null) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataEdit()
        {
            var w = new WPhoneTitleAddEdit(SelectedData) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataDelete()
        {
            ...
            var list = srh.Select(n => DynamicGridControl.GetRow(n) as DBBTPhoneTitle2).ToList();
                ...
                w =>
                {
                    ...
                    foreach (var db in list)
                    {
                        try
                        {
                            ...
                            var db2 = DBBTPhoneTitle2.FindByOid(dxs, db.Oid);
        ...
        private void Export()
        {
            var sf = new SaveFileDialog
            {
                ...
                FileName = "PhoneTitle.xml"
            };
            if (sf.ShowDialog() != true) return;

            try
            {
                var io = new PhoneTitleIO()
                {
                    Titles = (from n in DBBTPhoneTitle2.GetAll(ADatabase.Dxs) select n.Title).ToArray()
                };
        ...
        private void Import()
        {
            ...
            try
            {
                var serializer = new XmlSerializer(typeof(PhoneTitleIO));
                using (var f = new StreamReader(sf.FileName))
                {
                    var io = serializer.Deserialize(f) as PhoneTitleIO;
                    if (io == null)
                        throw new Exception("محتوای فایل معتبر نمی باشد.");
                    foreach (var v in io.Titles)
                    {
                        var dbcc = DBBTPhoneTitle2.FindDuplicateTitleExcept(ADatabase.Dxs, null, v);
                        if (dbcc != null) continue;
                        dbcc = new DBBTPhoneTitle2(ADatabase.Dxs) { Title = v };
                        dbcc.Save();
                    }
                }
                POLMessageBox.ShowInformation("عملیات با موفقیت انجام شد.", DynamicOwner);
        ...
        private void PopulateDataList()
        {
            ...
            XPCollection<DBBTPhoneTitle2> xpc = null;

            xpc = DBBTPhoneTitle2.GetAll(ADatabase.Dxs, ff);
