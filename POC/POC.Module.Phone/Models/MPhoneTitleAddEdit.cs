...
{
    public class MPhoneTitleAddEdit : NotifyObjectBase
    {
        ...
        private DBBTPhoneTitle2 DynamicSelectedData { get; set; }

        #region CTOR
        public MPhoneTitleAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "اطلاعات عنوان شماره تماس"; }
        }
        #endregion
        ...
        private bool Validate()
        {
            ...
            var db = DBBTPhoneTitle2.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData =new DBBTPhoneTitle2(uow)
                        {
                            Title = Title.Trim(),
                        };
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBBTPhoneTitle2.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
