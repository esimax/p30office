...
{
    public class MFactorReportTemplateManage : NotifyObjectBase
    {
        ...
        #region CTOR
        public MFactorReportTemplateManage(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "فهرست عناوین فاکتور"; }
        }
        #endregion
        ...
        #region DataList
        private XPCollection<DBACFactorReportTemplate> _DataList;
        public XPCollection<DBACFactorReportTemplate> DataList
        {
        ...
        #endregion
        #region SelectedData
        private DBACFactorReportTemplate _SelectedData;
        ...
        private void InitCommands()
        {
            ...
            CommandHelp = new RelayCommand(Help, () => ConstantPOCHelpURL.POCHelp52 != "");
        }

        private void DataNew()
        {
            var w = new WFactorReportTemplateAddEdit(null) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataEdit()
        {
            var w = new WFactorReportTemplateAddEdit(SelectedData) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataDelete()
        {
            ...
            var list = srh.Select(n => DynamicGridControl.GetRow(n) as DBACFactorReportTemplate).ToList();
                ...
                w =>
                {
                    ...
                    foreach (var db in list)
                    {
                        try
                        {
                            ...
                            var db2 = DBACFactorReportTemplate.FindByOid(dxs, db.Oid);
        ...
        private void Export()
        {
            var sf = new SaveFileDialog
            {
                ...
                FileName = "FactorTitle.xml"
            };
            if (sf.ShowDialog() != true) return;

            try
            {
                var io = new FactorTitleIO()
                {
                    Titles = (from n in DBACFactorReportTemplate.GetAll(ADatabase.Dxs) select n.Title).ToArray()
                };
        ...
        private void Import()
        {
            ...
            try
            {
                var serializer = new XmlSerializer(typeof(FactorTitleIO));
                using (var f = new StreamReader(sf.FileName))
                {
                    var io = serializer.Deserialize(f) as FactorTitleIO;
                    if (io == null)
                        throw new Exception("محتوای فایل معتبر نمی باشد.");
                    foreach (var v in io.Titles)
                    {
                        var dbcc = DBACFactorReportTemplate.FindDuplicateTitleExcept(ADatabase.Dxs, null, v);
                        if (dbcc != null) continue;
                        dbcc = new DBACFactorReportTemplate(ADatabase.Dxs) { Title = v };
                        dbcc.Save();
                    }
                }
                POLMessageBox.ShowInformation("عملیات با موفقیت انجام شد.", DynamicOwner);
        ...
        private void PopulateDataList()
        {
            ...
            XPCollection<DBACFactorReportTemplate> xpc = null;

            xpc = DBACFactorReportTemplate.GetAll(ADatabase.Dxs, ff);
        ...
        private void Help()
        {
            System.Diagnostics.Process.Start(ConstantPOCHelpURL.POCHelp52);
        }
        #endregion
