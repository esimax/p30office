...
{
    public class MFactorTitleManage : NotifyObjectBase
    {
        ...
        #region CTOR
        public MFactorTitleManage(object mainView)
        {
            MainView = mainView;
        ...
        #region DataList
        private XPCollection<DBBTFactorTitle2> _DataList;
        public XPCollection<DBBTFactorTitle2> DataList
        {
        ...
        #endregion
        #region SelectedData
        private DBBTFactorTitle2 _SelectedData;
        ...
        {
            var w = new WFactorTitleAddEdit(null) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataEdit()
        {
            var w = new WFactorTitleAddEdit(SelectedData) { Owner = DynamicOwner };
            if (w.ShowDialog() == true)
                DataRefresh();
        }
        private void DataDelete()
        {
            ...
            var list = srh.Select(n => DynamicGridControl.GetRow(n) as DBBTFactorTitle2).ToList();
                ...
                w =>
                {
                    ...
                    foreach (var db in list)
                    {
                        try
                        {
                            ...
                            var db2 = DBBTFactorTitle2.FindByOid(dxs, db.Oid);
        ...
        private void Export()
        {
            ...
            try
            {
                ...
                {
                    Titles = (from n in DBBTFactorTitle2.GetAll(ADatabase.Dxs) select n.Title).ToArray()
                };
        ...
        private void Import()
        {
            ...
            try
            {
                ...
                using (var f = new StreamReader(sf.FileName))
                {
                    ...
                    {
                        var dbcc = DBBTFactorTitle2.FindDuplicateTitleExcept(ADatabase.Dxs, null, v);
                        if (dbcc != null) continue;
                        dbcc = new DBBTFactorTitle2(ADatabase.Dxs) { Title = v };
                        dbcc.Save();
                    }
                }
                POLMessageBox.ShowInformation("عملیات با موفقیت انجام شد.", DynamicOwner);
        ...
        private void PopulateDataList()
        {
            ...
            XPCollection<DBBTFactorTitle2> xpc = null;

            xpc = DBBTFactorTitle2.GetAll(ADatabase.Dxs, ff);
