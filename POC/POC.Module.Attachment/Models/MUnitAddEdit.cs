...
{
    public class MUnitAddEdit : NotifyObjectBase
    {
        ...
        private DBBTUnit DynamicSelectedData { get; set; }

        #region CTOR
        public MUnitAddEdit(object mainView)
        {
            MainView = mainView;
        ...
        {
            get { return "اطلاعات واحد اندازه گیری"; }
        }
        #endregion
        ...
        private bool Validate()
        {
            ...
            var db = DBBTUnit.FindDuplicateTitleExcept(ADatabase.Dxs, DynamicSelectedData, HelperConvert.CorrectPersianBug(Title.Trim()));
            if (db != null)
            {
        ...
        private bool Save()
        {
            try
            {
                if (DynamicSelectedData == null)
                {
                    ...
                    {
                        DynamicSelectedData =new DBBTUnit(uow)
                        {
                            Title = Title.Trim(),
                        };
                        uow.CommitChanges();
                    }
                    DynamicSelectedData = DBBTUnit.FindByOid(ADatabase.Dxs, DynamicSelectedData.Oid);
                }
                else
                {
